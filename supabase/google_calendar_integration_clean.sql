-- Google Calendar Integration Migration (Clean Install)
-- 既存のテーブルとポリシーをクリーンアップしてから再作成

-- 1. 既存のポリシーを削除（エラーを無視）
DROP POLICY IF EXISTS "Users can view their own calendar settings" ON user_calendar_settings;
DROP POLICY IF EXISTS "Users can insert their own calendar settings" ON user_calendar_settings;
DROP POLICY IF EXISTS "Users can update their own calendar settings" ON user_calendar_settings;
DROP POLICY IF EXISTS "Users can delete their own calendar settings" ON user_calendar_settings;
...
DROP TABLE IF EXISTS user_calendar_settings CASCADE;
...
CREATE INDEX idx_calendar_sync_log_task_id ON calendar_sync_log(task_id);
CREATE INDEX idx_calendar_sync_log_google_event_id ON calendar_sync_log(google_event_id);
CREATE INDEX idx_calendar_sync_log_synced_at ON calendar_sync_log(synced_at DESC);
...
-- 9. google_event_id のインデックス（高速検索用）
